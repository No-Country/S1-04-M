...
transactionsCtrl.createNewTransaction = async (req, res) => {
  ...
  const newBalance = await Card.Card.findOneAndUpdate(filter_origin, {
    $inc: {
      balance: -amount
    }
  }, {
  ...
  const newDestinyCard = await Card.Card.findOneAndUpdate(filter_destiny, {
    $inc: {
      balance: amount
    }
  }, {
