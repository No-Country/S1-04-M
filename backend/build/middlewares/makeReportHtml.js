...
makeReportHtml.create = async function (transactions, month = undefined) {
  try {
    var fileNameGlobal = month === undefined ? `Todas-las-transacciones-${new Date().getTime()}.pdf` : `transacciones-${this.meses[month]}-${new Date().getTime()}.pdf`;
    const title = month === undefined ? "Todas las transacciones" : `Transacciones de ${this.meses[month].toLowerCase()}`;
    const bodyInit = `
      <body>
        <h2>${title}</h2>
        <ul>
    ...
    <li>
      ...
      <br>
      <div>
      <span>Fecha: ${transaction.created_at ? `${transaction.created_at}`.slice(0, 10) : "-"}</span>
    ...
    const pdfMakerPromise = new Promise((resolve, reject) => {
      pdf.create(content).toFile(`./public/pdf/${fileNameGlobal}`, (err, res) => {
        if (err) {
